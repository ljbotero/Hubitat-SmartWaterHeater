...
metadata {
    definition (name: "ZigBee Dimmer", namespace: "smartthings", author: "SmartThings", ocfDeviceType: "oic.d.light", runLocally: true, minHubCoreVersion: '000.019.00012', executeCommandsLocally: true) {
        ...
        fingerprint profileId: "0104", inClusters: "0000, 0003, 0004, 0005, 0006, 0008, 0300, 0B04, 0B05, FC01, FC08", outClusters: "0003, 0019", manufacturer: "LEDVANCE", model: "E11-G13", deviceJoinName: "SYLVANIA E11-G13 A19 Soft White"
...
def parse(String description) {
    ...
    if (event) {
        ...
        } else if (device.getDataValue("manufacturer") == "LEDVANCE") {
          if (descMap && descMap.command == "00") {
            runInMillis(250, off)
            runInMillis(500, on)
          }
        } else {
          log.warn "DID NOT PARSE MESSAGE for description: $description, command: ${descMap.command}"
        }
    }
}
